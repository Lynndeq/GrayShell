...
powershell /c write-host -back darkgray -fore white -nonewline GrayShell
powershell /c write-host -back black -fore black -nonewline Z
powershell /c write-host -back red -fore white 1.1.1-RAW
echo.
goto cmd
:curdir
cls
...
if %suinput% == ls goto ls
if %suinput% == dir goto ls
if %suinput% == plugin goto plg
...
goto cmd
:plg
if exist %appdata%\grshplugins\placeholder (
    goto plgenter
) else (
    goto plgcreate
)
:plgcreate
md %appdata%/grshplugins
cd %appdata%/grshplugins
echo 000-000-PLUGINS-000-000 > placeholder
goto plgenter
:plgenter
cd %appdata%/grshplugins
cmd /c "%appdata%/grshplugins/pluginloader.exe"
pause >NUL
exit
